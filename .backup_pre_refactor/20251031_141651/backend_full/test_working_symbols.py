...
def test_hyperliquid_markets():
    ...
    # Separate by type
    perpetual_markets = {k: v for k, v in usdc_markets.items() if v.get('type') == 'swap' or ':' in k}
    spot_markets = {k: v for k, v in usdc_markets.items() if v.get('type') == 'spot' and ':' not in k}
