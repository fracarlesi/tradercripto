...
try:
    ...
    if account:
        ...
        print(f"   API Key: {account.api_key[:20]}..." if len(account.api_key) > 20 else f"   API Key: {account.api_key}")
        ...
        if account.api_key in ["default-key-please-update-in-settings", "default", "", None]:
            ...
            print(f"   Please update the API key in the settings to enable automated trading.\n")
        else:
