    ...
    try:
        ...
        for account in accounts:
            try:
                ...
                if operation == "buy":
                    ...
                    if order_value < MIN_ORDER_SIZE:
                        if available_cash >= MIN_ORDER_SIZE:
                            logger.info(f"Order value ${order_value:.2f} below minimum, adjusting to ${MIN_ORDER_SIZE}")
                            order_value = MIN_ORDER_SIZE
                        else:
                            logger.info(f"Insufficient funds for minimum order size (${MIN_ORDER_SIZE}) for {account.name}, skipping")
                            save_ai_decision(db, account, decision, portfolio, executed=False)
                            continue
                ...
                logger.warning(f"🔴 EXECUTING REAL ORDER on Hyperliquid: {side} {symbol} quantity={quantity}")
