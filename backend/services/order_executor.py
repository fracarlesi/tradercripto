...
def place_and_execute(
    ...
    exec_price = Decimal(
        str(price if (order_type == "LIMIT" and price) else get_last_price(symbol, market))
    )
    ...
    if side == "BUY":
        ...
        pos = (
            db.query(Position)
            .filter(
                Position.user_id == user.id, Position.symbol == symbol, Position.market == market
            )
            .first()
        )
        if not pos:
        ...
        new_cost = (Decimal(str(pos.avg_cost)) * Decimal(int(pos.quantity)) + notional) / Decimal(
            new_qty
        )
