...
async def create_new_order(request: OrderCreateRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if request.session_token:
            ...
            raise HTTPException(
                status_code=400, detail="Please provide either session token or username+password"
            )
...
async def get_user_pending_orders(user_id: int | None = None, db: Session = Depends(get_db)):
    """
    Get pending orders
...
async def get_user_orders(user_id: int, status: str | None = None, db: Session = Depends(get_db)):
    """
...
async def execute_order_manually(order_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if executed:
            return OrderExecutionResult(
                order_id=order_id, executed=True, message="Order executed successfully"
            )
        else:
            return OrderExecutionResult(
                order_id=order_id,
...
async def cancel_user_order(
    order_id: int, reason: str = "User cancelled", db: Session = Depends(get_db)
):
    """
    Cancel an order
    ...
    try:
        ...
        if order.status != "PENDING":
            raise HTTPException(
                status_code=400, detail=f"Order status is {order.status}, cannot be cancelled"
            )
