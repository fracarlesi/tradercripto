...
from schemas.user import UserAuthResponse, UserCreate, UserLogin, UserOut, UserUpdate
...
async def update_user_profile(
    session_token: str, user_data: UserUpdate, db: Session = Depends(get_db)
):
    try:
        user_id = verify_auth_session(db, session_token)
        if not user_id:
        ...
        user = update_user(
            db=db, user_id=user_id, username=user_data.username, email=user_data.email
        )
