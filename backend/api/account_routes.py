...
async def update_account_settings(account_id: int, payload: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if "api_key" in payload:
            ...
            logger.info(
                f"Updated api_key (length: {len(payload['api_key']) if payload['api_key'] else 0})"
            )
...
async def get_asset_curve_by_timeframe(timeframe: str = "1d", db: Session = Depends(get_db)):
    ...
    try:
            ...
            for i, ts in enumerate(timestamps):
                ...
                for trade in trades:
                    ...
                    if trade_time <= ts_datetime:
                        # Update cash
                        trade_amount = float(trade.price) * float(trade.quantity) + float(
                            trade.commission
                        )
                        if trade.side == "BUY":
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get asset curve for timeframe: {str(e)}"
        )
...
async def test_llm_connection(payload: dict):
    ...
    try:
        ...
        try:
            ...
            return {
                "success": False,
                "message": f"Failed to connect to {base_url}. Please check the base URL.",
            }
...
@router.post("/{account_id}/sync-hyperliquid")
async def sync_account_with_hyperliquid(account_id: int, db: Session = Depends(get_db)):
    """
    Manually trigger synchronization of account balance, positions, and trades from Hyperliquid.

    This endpoint allows forcing a sync to ensure local database matches the
    authoritative on-chain state from Hyperliquid DEX.

    Returns:
        - success: bool
        - total_equity: float (if successful)
        - available: float (if successful)
        - margin_used: float (if successful)
        - balance_diff: float (difference from previous balance)
        - positions_synced: int (number of positions synchronized)
        - trades_synced: int (number of fills/trades synchronized)
        - error: str (if failed)
    """
    try:
        ...
        from services.hyperliquid_sync_service import sync_specific_account

        result = sync_specific_account(account_id)

        if not result.get("success"):
            error_msg = result.get("error", result.get("reason", "Unknown error"))
            raise HTTPException(status_code=400, detail=f"Sync failed: {error_msg}")

        return {
            "success": True,
            "message": f"Account '{account.name}' synchronized with Hyperliquid",
        ...
        logger.error(f"Failed to sync account {account_id}: {e}", exc_info=True)
