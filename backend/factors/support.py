...
def calculate_days_from_longest_candle(df_window):
    ...
    body_lengths = (
        (df_window.iloc[1:]["Close"] - df_window.iloc[1:]["Open"]).abs() * 100 / first_close
    )
...
def compute_support(
    history: dict[str, pd.DataFrame], top_spot: pd.DataFrame | None = None, window_size: int = 60
) -> pd.DataFrame:
...
def compute_support_with_default_window(
    history: dict[str, pd.DataFrame], top_spot: pd.DataFrame | None = None
) -> pd.DataFrame:
...
SUPPORT_FACTOR = Factor(
    ...
    columns=[
        ...
        {"key": "Support Score", "label": "Support Score", "type": "score", "sortable": True},
        {
            ...
            "label": f"{DEFAULT_WINDOW_SIZE} Days From Longest Candle",
            "type": "number",
            "sortable": True,
        },
    ],
