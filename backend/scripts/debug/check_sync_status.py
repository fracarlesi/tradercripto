...
def check_sync_status():
    ...
    exchange = Exchange(
        wallet=wallet, base_url=constants.MAINNET_API_URL, account_address=wallet.address
    )
    ...
    try:
        ...
        if db_orders:
            ...
            for order in db_orders[:10]:
                print(
                    f"  - {order.symbol}: {order.side} {order.quantity} @ ${order.price} (Status: {order.status}, Created: {order.created_at})"
                )
        ...
        if hl_fills:
            ...
            for fill in hl_fills[:10]:
                ...
                if isinstance(time, int):
                    dt = datetime.fromtimestamp(time / 1000, tz=UTC)
            ...
            if hl_fills:
                ...
                if isinstance(last_time, int):
                    last_dt = datetime.fromtimestamp(last_time / 1000, tz=UTC)
                    print(
        ...
        if db_trades:
            ...
            for trade in db_trades[:10]:
                print(
                    f"  - {trade.symbol}: {trade.side} {trade.quantity} @ ${trade.price} ({trade.trade_time})"
                )
