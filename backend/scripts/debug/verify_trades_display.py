...
def simulate_snapshot_for_user(username: str):
    ...
    try:
        ...
        if orders:
            for o in orders:
                print(
                    f"  • {o.order_no}: {o.side} {o.quantity} {o.symbol} @ {o.price or 'MARKET'} - {o.status}"
                )
        else:
            print("  No orders")
        ...
        if trades:
            ...
            for i, t in enumerate(trades[:10], 1):
                print(
                    f"  {i:2d}. {t.trade_time} | {t.side:4s} {t.quantity:>8} {t.symbol:5s} @ ${t.price:>10.4f} | Fee: ${t.commission:.2f}"
                )
        else:
