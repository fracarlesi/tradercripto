...
# =============================================================================
# Removed in phase 4: legacy ``Config``, ``ServicesConfig``,
# ``ExecutionEngineConfig``, ``ConfigLoader``, ``load_config``,
# ``get_config``, ``reload_config``. The production path uses
# ``crypto_bot.main.ConservativeConfig`` for the bot dataclass and
# ``BotConfig`` (below) for the execution-engine runtime config.
# Nested models (SystemConfig, HyperliquidConfig, RiskConfig, LLMConfig,
# ...) are still defined above for callers that import them directly.
# =============================================================================
...
class BotConfig(BaseModel):
    ...
    def from_conservative(cls, cfg: Any) -> "BotConfig":
        ...
        exec_cfg = BotExecutionConfig(
            ...
            retry_attempts=cfg.execution_max_retries,
            retry_delay_seconds=cfg.execution_retry_delay_seconds,
